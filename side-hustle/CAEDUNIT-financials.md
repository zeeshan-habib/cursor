# CAEDUNIT INC. — Corporate Financial Overview

| | |
|---|---|
| **Corporation** | CAEDUNIT INC. |
| **BN** | 779161207 |
| **Incorporated** | September 2021 (Ontario) |
| **Fiscal Year End** | August 31 |
| **Bank Account** | RBC Chequing #06702-1065374 |
| **GST/HST Account** | Active (confirmed — CRA issued $244 refund Jan 2023) |

---

## Financial Statements — Google Sheet

**[CAEDUNIT INC. — Financial Statements (Draft for CPA)](https://docs.google.com/spreadsheets/d/1kGmxI7IHrVTEdZjXIczYbaWwePnjHyRKEUHfynQ9T4A/edit)**

> Prepared 2026-05-12 from RBC bank statements (full transaction history Oct 2021 – May 2026).
> Status: DRAFT — for CPA review. Items marked [VERIFY] need receipts or bank confirmation.
> ⚠️ Known issue: rows containing `====` show formula errors in Google Sheets — delete those rows (they're visual separators, not data).

---

## Summary — All Fiscal Years

| Fiscal Year | Revenue | Net Income (Worst Case) | Est. Tax (12.2%) | T2 Status |
|---|---|---|---|---|
| FY2021/22 | $2,745.73 | $1,712.30 | Up to $208.90 | Overdue — paper T2, CPA required |
| FY2022/23 | $0.00 | -$208.81 | $0.00 | Overdue — CPA required |
| FY2023/24 | $4.28 | -$35.37 | $0.00 | Overdue — self-file (nil) |
| FY2024/25 | $5.37 | $5.37 | $0.66 | **OVERDUE NOW** (was due Feb 28 2026) |
| FY2025/26 | $3.03 | -$16.97 | $0.00 | Current — not yet due (due Feb 28 2027) |

> **Total maximum tax exposure across all years: ~$209.56** (almost entirely FY2021/22 worst case).
> If unclassified FY2021/22 transfers are confirmed as business expenses: tax owed = $0 for all years.

---

## Filing Sequence

First T2 must be paper-filed; subsequent years unlock CRA EFILE online filing.

1. **FY2021/22** — Paper T2 (CPA). Revenue $2,745.73 PayPal; COGS $840 (TransferWise India $600 + ATM cash $240); operating expenses $193.43. Six unclassified outgoing transfers totalling $1,790.45 — call RBC to classify before filing.
2. **FY2022/23** — T2 with CPA. Net loss $208.81; no tax. Two unclassified transfers ($480.99 total). GST refund $244 received — not income.
3. **FY2023/24** — Self-file nil T2. Interest income $4.28; expenses $39.65; net loss $35.37; no tax.
4. **FY2024/25** — Self-file T2 immediately (overdue). Interest income $5.37; no expenses; net income $5.37; tax $0.66.
5. **FY2025/26** — Current year. Interest income $3.03 YTD; expenses $20.00 (inactive account fee Mar 2026); net loss $16.97; no tax.

---

## Pre-CPA Action Items

| # | Action | Amount | Impact |
|---|---|---|---|
| 1 | Call RBC — get destination accounts for 6 FY2021/22 online transfers | $1,790.45 total | Vendor = deductible; personal = draw. Determines if tax is $209 or $0. |
| 2 | Find Dec 13 2021 flight booking email | $900 | Business trip = deductible |
| 3 | Find accountant invoice / email (paid ~$400, Jan 2023) | $400 | Professional fee = deductible |
| 4 | Write brief memo for ATM cash withdrawal (Mar 2022) | $240 | Cash vendor payment — memo is sufficient, no receipt needed |
| 5 | Log into CRA My Business Account (BN 779161207) | — | Verify which T2s CRA shows as outstanding before filing |
| 6 | Fix Google Sheet formula errors | — | Delete rows with `====` separators; share with CPA |

---

## Balance Sheet Snapshot — Aug 31 2025

| Item | Amount |
|---|---|
| Cash (RBC Chequing) | $257.53 |
| **Total Assets** | **$257.53** |
| Shareholder Loan Payable (Zee Habib) | $770.00 |
| **Total Liabilities** | **$770.00** |
| Share Capital | $1.00 *(unconfirmed — check articles of incorporation)* |
| Retained Earnings (Deficit) | -$513.47 |
| **Total Equity** | **-$512.47** |

---

## Shareholder Loan History

Zee transferred a total of $770 into the corporation over three fiscal years. These are loans from Zee to CAEDUNIT (shareholder loans payable) — not income to the corporation.

| Fiscal Year | Amount Transferred | Cumulative Balance |
|---|---|---|
| FY2021/22 | $120.00 | $120.00 |
| FY2022/23 | $400.00 | $520.00 |
| FY2023/24 | $250.00 | $770.00 |
| FY2024/25–FY2025/26 | $0.00 | $770.00 |

> CAEDUNIT owes Zee $770. This should appear as a liability on the T2 balance sheet for each year.

---

## Homebase Freelance Contract (May 2026)

- **Client:** Homebase (former employer)
- **Rate:** $100/hr
- **Scope:** Build MSHR context file for Claude Code (CMO Katie + CRO Ray engagement)
- **Income type:** T4A (self-employment / contractor) — report on T1 Schedule T2125
- **Income earned:** ~$1,300 gross (May 2026)
- **Employment agreement check (MaintainX):** No non-compete clause, no outside work prohibition — confirmed clean.
- **NDA check (MaintainX):** Proprietary property clause covers only work "relating to the Business of the Company" (industrial CMMS). Homebase = SMB workforce management. Unrelated — confirmed clean.
- **Billing:** Personal income for now. Route through CAEDUNIT once T2s are filed and corp is active.

### MacBook Air M5 Purchase Tracker

**Status:** Waiting for deal — do not buy at Apple.ca list price unless urgent.

| Field | Detail |
|---|---|
| **Target config** | 13-inch MacBook Air M5, 16 GB RAM, 512 GB SSD |
| **Replacing** | Personal M1 MacBook Air (2020) — functional, not urgent |
| **Work machine** | M4 MacBook Air (MaintainX) — separate; no upgrade needed |
| **Target pre-tax price** | **≤ $1,399** ($100+ off list) |
| **List price (Apple.ca)** | $1,499 + 13% HST = **~$1,694** total |
| **Hard deadline** | **July 31, 2026** — reassess if no deal by then |
| **Watch window** | Late June – mid-July (back-to-school, Amazon Prime Day) |

**Where to buy (in order):**

| Priority | Retailer | Card | Why |
|---|---|---|---|
| 1 | **Amazon.ca** (when in stock, $100+ off) | **MBNA Amazon Mastercard** (~2.5% cashback) | ~$100 discount + ~$35 cashback ≈ **$135 saved** vs Apple.ca |
| 2 | **Best Buy / Staples** (if matching Amazon pricing) | MBNA Amazon or Amex Simply Cash | Same logic — only if ≤ $1,399 pre-tax |
| 3 | **Apple.ca** (fallback only) | **Amex Simply Cash** (2% cashback) | No retailer discount; ~$34 back |

**Cards to avoid for this purchase:**

| Card | Reason |
|---|---|
| Wealthsimple VIP Infinite Privilege | $6K limit; $3K rent + ~$1K used = ~$2K available. A ~$1,700 charge spikes utilization and hurts TransUnion. |
| RBC Avion / WestJet / Simplii | ≤1.5% — worse than Simply Cash or MBNA |
| Amex Cobalt | 1x MR on electronics unless welcome-bonus month ($750 spend → 1,250 MR bonus still in year 1) |

**Action items:**

- [ ] Set price/stock alerts on Amazon.ca and Best Buy for M5 16GB/512
- [ ] Buy same day when ≤ $1,399 pre-tax and in stock
- [ ] Save receipt + confirmation email to 2026 tax folder
- [ ] Write business-use memo on purchase date (see tax section below)
- [ ] Update this tracker with actual purchase date, price, and card used

---

### Tax Treatment (Personal T1 — 2026 Tax Year)

**MacBook Air Upgrade — Business Equipment Deduction**

Purchase must occur in **calendar year 2026** to offset 2026 T4A income on T2125 (filed spring 2027). No rush — any 2026 purchase date works.

**Estimated at Apple list price ($1,499 pre-tax):**

| Item | Detail |
|---|---|
| Pre-tax cost | ~$1,499 CAD (update with actual receipt) |
| Ontario HST (13%) | ~$195 |
| **Total capital cost** | **~$1,694 CAD** |
| CCA Class | Class 50 (computers, 55% rate) |
| Filing method | Immediate expensing election — full business-use portion in year 1 |

**Business use % — claim honestly:**

Dedicated M4 Air covers MaintainX work. This M5 replaces personal M1. CRA expects **actual use**, not maximum allocation.

| Scenario | Business use % | Deductible (on $1,499) | Tax savings (~32%) |
|---|---|---|---|
| Mostly personal; occasional Homebase | **20–30%** | ~$300–450 | ~$96–144 |
| Split personal + regular Homebase consulting | **40–50%** | ~$600–750 | ~$192–240 |
| Primarily Homebase machine (aggressive — document heavily) | **80%** | ~$1,199 | ~$384 |

**Net income illustration (at 30% business use, list price, ~$1,300 T4A income):**

| Item | Amount |
|---|---|
| Gross T4A income | $1,300.00 |
| MacBook Air deduction (30%) | -$450.00 |
| Net self-employment income | ~$850.00 |
| Estimated tax + CPP (~32%) | ~$272.00 |
| MBNA cashback (~2.5% on ~$1,582 Amazon) | -~$40.00 |
| **Effective after-tax laptop cost** | **~$1,382** |

> Tax break is a sweetener, not the primary reason to buy. Do not inflate business % beyond defensible actual use.

**Filing notes:**
- File T2125 (Statement of Business or Professional Activities) with 2026 T1 return
- Claim laptop under CCA Class 50 with immediate expensing (max $100K for individuals — well under cap)
- Keep receipt — store with 2026 tax documents
- Business purpose memo (update date on purchase): *"MacBook Air M5 — [X]% business use for Homebase consulting (T4A). Personal M1 replaced. M4 Air remains primary MaintainX work machine."*
- Trade-in of old M1 MacBook Air: personal asset (no prior CCA claimed) — no tax event on disposal
- HST not claimable as ITC — under $30K small supplier threshold, no HST number

---

*Prepared: 2026-05-12 | Updated: 2026-05-24*
